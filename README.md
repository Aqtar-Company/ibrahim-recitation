...
## هل يصل التحديث إلى من أخذ البيانات؟

**لا يصل من نفسه.** git لا يدفع إلى أحد؛ الناس هم الذين يسحبون. فمن نسخ
`segments.csv` إلى تطبيقه في يناير يبقى على نسخة يناير حتى يسحب هو.

وذلك يختلف باختلاف ما أخذه:

| ما أخذه | هل يصله التحديث؟ |
|---|---|
| نسخ `data/` داخل تطبيقه | **لا.** حتى يسحب بنفسه — وهذا حال أغلب من يدمج |
| يقرأ `data/` من رابطٍ حيّ (أدناه) | **نعم**، عند كل تشغيل |
| يشغّل الصوت من روابط ibrahimquran.com | **نعم** للصوت — واحذر: تعديلُ الصوت يُفسد التوقيتات عند الجميع فورًا |
| قصّ الصوت ورفعه على خادمه | **لا**، لا للصوت ولا للبيانات |

### رابطٌ حيّ لمن أراد المتابعة

```
https://cdn.jsdelivr.net/gh/Aqtar-Company/ibrahim-recitation@v1.1.0/data/segments.json
https://cdn.jsdelivr.net/gh/Aqtar-Company/ibrahim-recitation@latest/data/segments.json
```

`@v…` يثبّت نسخةً بعينها فلا يتغيّر تحتك شيء، و`@latest` يتبع آخر وسم.
واقرأ `version` داخل `segments.json` نفسه: الملفّ يُنسَخ ويُفارق المستودع،
فالرقم فيه لا في مكانٍ آخر.
...
ثم ارفع النسخة في `VERSION` واكتب في `CHANGELOG.md` أيُّ الأوجه تغيّر —
من عنده النسخة القديمة لا يعلم أنّ عليه السحب إلا أن تقول له.
